...
const (
	...
	//修改设备视频加密密码
	UPDATESBVIEWPASSWORD="https://open.ys7.com/api/lapp/device/password/update"
	//生成设备扫描配网二维码二进制数据
	...
	//根据序列号获取设备的状态信息
	//根据设备序列号查询设备能力集
	SELECTPOWER="https://open.ys7.com/api/lapp/device/capacity"
...
func (ys *Ys7) AddDevice(deviceSerial, validateCode string) (err error) {
	/*device:=make([]Device,10)
	device,_=ys.GetAllDeviceList()
	dev:=device[0].DeviceSerial
	deviceSerial=dev*/
...
//修改设备视频加密密码
func (ys *Ys7)Updateviewpassword(accessToken,deviceSerial,oldPassword,newPassword string)(err error)  {
	params:=make(map[string]interface{})
	params["deviceSerial"]=deviceSerial
	params["oldPassword"]=oldPassword
	params["newPassword"]=newPassword
	_,err=ys.authorizeRequset("POST",UPDATESBVIEWPASSWORD,params,nil)
	if err!=nil {
		return
	}
	return nil
